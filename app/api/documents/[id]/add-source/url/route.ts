...
export async function POST(
  ...
  try { new URL(url); } catch { return new Response(JSON.stringify({ error: "Invalid URL" }), { status: 400 }); }

  const { text } = await extractTextFromURL(url);
  if (!text.trim()) return new Response(JSON.stringify({ error: "Could not extract text from this URL" }), { status: 422 });

  const chunkTexts = chunkText(text);
  if (chunkTexts.length === 0) return new Response(JSON.stringify({ error: "Page is too short" }), { status: 422 });
